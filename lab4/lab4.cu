...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
	...
	__shared__ unsigned char RMatrix[5][262];
    __shared__ unsigned char GMatrix[5][262];
    __shared__ unsigned char BMatrix[5][262];
    float val[Z][3];
        ...
        for (int x = threadIdx.x; x < width; x += blockDim.x) {
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                if (bound_check(y + v, 0, height)) {
                    RMatrix[v+yBound][threadIdx.x + 2] = s[channels * (width * (y + v) + x) + 2];
                    GMatrix[v+yBound][threadIdx.x + 2] = s[channels * (width * (y + v) + x) + 1];
                    BMatrix[v+yBound][threadIdx.x + 2] = s[channels * (width * (y + v) + x) + 0];
                    if (threadIdx.x == 0) {
                        if (bound_check(x - 2, 0, width)) {
                            RMatrix[v+yBound][threadIdx.x] = s[channels * (width * (y + v) + x-2) + 2];
                            GMatrix[v+yBound][threadIdx.x] = s[channels * (width * (y + v) + x-2) + 1];
                            BMatrix[v+yBound][threadIdx.x] = s[channels * (width * (y + v) + x-2) + 0];
                        }
                        if (bound_check(x - 1, 0, width)) {
                            RMatrix[v+yBound][threadIdx.x + 1] = s[channels * (width * (y + v) + x-1) + 2];
                            GMatrix[v+yBound][threadIdx.x + 1] = s[channels * (width * (y + v) + x-1) + 1];
                            BMatrix[v+yBound][threadIdx.x + 1] = s[channels * (width * (y + v) + x-1) + 0];
                        }
                    } else if (threadIdx.x == blockDim.x - 1) {
                        if (bound_check(x + 1, 0, width)) {
                            RMatrix[v+yBound][threadIdx.x + 3] = s[channels * (width * (y + v) + x+1) + 2];
                            GMatrix[v+yBound][threadIdx.x + 3] = s[channels * (width * (y + v) + x+1) + 1];
                            BMatrix[v+yBound][threadIdx.x + 3] = s[channels * (width * (y + v) + x+1) + 0];
                        }
                        if (bound_check(x + 2, 0, width)) {
                            RMatrix[v+yBound][threadIdx.x + 4] = s[channels * (width * (y + v) + x+2) + 2];
                            GMatrix[v+yBound][threadIdx.x + 4] = s[channels * (width * (y + v) + x+2) + 1];
                            BMatrix[v+yBound][threadIdx.x + 4] = s[channels * (width * (y + v) + x+2) + 0];
                        }
                    }
                }
            }
                ...
                for (int v = -yBound; v <= yBound; ++v) {
                    for (int u = -xBound; u <= xBound; ++u) {
                        if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                            ...
                            const unsigned char R = RMatrix[v+yBound][threadIdx.x + 2 + u];
                            const unsigned char G = GMatrix[v+yBound][threadIdx.x + 2 + u];
                            const unsigned char B = BMatrix[v+yBound][threadIdx.x + 2 + u];
