...
__constant__ const int mask[MASK_N][MASK_X][MASK_Y] = { 
    {{ -1, -4, -6, -4, -1},
     { -2, -8,-12, -8, -2},
     {  0,  0,  0,  0,  0}, 
     {  2,  8, 12,  8,  2}, 
...
int main(int argc, char** argv) {
    ...
    // Declared device variables
    unsigned *device_height, *device_width, *device_channels;
    unsigned char *device_s, *device_t;

    // Malloc memory for device 
    cudaMalloc((void **)&device_height, sizeof(unsigned));
    cudaMalloc((void **)&device_width, sizeof(unsigned));
    cudaMalloc((void **)&device_channels, sizeof(unsigned));
    cudaMemcpy(device_height, &height, sizeof(unsigned), cudaMemcpyHostToDevice);
    cudaMemcpy(device_width, &width, sizeof(unsigned), cudaMemcpyHostToDevice);
    cudaMemcpy(device_channels, &channels, sizeof(unsigned), cudaMemcpyHostToDevice);
    ...
    // Decide num_blocks, block_size
    const int num_blocks = 256;
    const int block_size = height/256 + 1;
